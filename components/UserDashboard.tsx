  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700">
      <section className="space-y-6">
        <div className="flex flex-col sm:flex-row items-center justify-between border-b-2 border-[#C8102E] pb-3 px-1 gap-4">
              ...
              >
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="flex flex-col">
                    <span className="text-[0.5625rem] font-black text-[#003366] uppercase tracking-[0.2em]">{t.tracking}</span>
                    <span className="text-sm font-black text-slate-800 tracking-tight">{order.trackingId}</span>
                  </div>
                  <div className={`text-[0.5rem] font-black px-2.5 py-1 rounded-sm uppercase tracking-widest shadow-sm ${
                ...
                <div className="grid grid-cols-2 gap-2 mt-auto">
                  {hasGrievance ? (
                    <div className="col-span-2 py-2 bg-emerald-50 text-emerald-700 border border-emerald-200 text-[0.5625rem] font-black uppercase tracking-widest rounded-sm flex items-center justify-center gap-2">
                      <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" strokeWidth={2}/></svg>
                      Grievance Logged
                    </div>
                  ) : (
                    <>
                      <button 
                        ...
                        className={`py-2 text-[0.5625rem] font-black uppercase tracking-widest rounded-sm transition-all border ${
          ...
          <div className="bg-white border border-slate-200 rounded-sm shadow-xl overflow-hidden sticky top-32">
            <div className={`px-8 py-4 border-b-2 flex items-center justify-between ${mode === 'Complaint' ? 'bg-[#C8102E] border-[#C8102E]' : 'bg-[#003366] border-[#003366]'}`}>
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-[0.6875rem] font-black text-white uppercase tracking-[0.2em]">
                  {mode === 'Complaint' ? t.registerComplaint : t.shareFeedback}
                </h2>
              </div>
              {selectedOrder && (
                <button onClick={() => {setSelectedOrder(null); setSubject('');}} className="text-[0.5625rem] font-black text-[#FFCC00] hover:underline uppercase tracking-widest">
                  Reset Form
                </button>
              )}
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-10 space-y-8 bg-white relative">
              ...
              <div className="space-y-2">
                <label className="text-[0.625rem] font-black text-[#003366] uppercase tracking-widest">Subject Line</label>
                <input 
                  type="text" 
                  value={subject}
              ...
              <div className="space-y-2">
                <label className="text-[0.625rem] font-black text-[#003366] uppercase tracking-widest">
              ...
              <div className="pt-2">
                 <button 
                    ...
                    className={`w-full py-4 text-white text-[0.6875rem] font-black uppercase tracking-[0.3em] rounded-sm shadow-2xl transition-all active:scale-95 border-b-4 border-black/10 ${mode === 'Complaint' ? 'bg-[#C8102E] hover:brightness-110' : 'bg-[#003366] hover:brightness-110'}`}
                  >
            ...
            <div className="p-8 overflow-y-auto space-y-8 max-h-[800px] bg-[#f8fafc]">
              {userComplaints.length === 0 ? (
                <div className="text-center py-40 opacity-20 flex flex-col items-center gap-6">
                  <p className="text-[0.625rem] font-black uppercase tracking-[0.4em] text-[#003366]">History Empty</p>
                </div>
              ) : (
                userComplaints.map(complaint => (
                  <div key={complaint.id} className="bg-white border border-slate-200 rounded-sm p-8 space-y-5 hover:border-[#C8102E]/30 shadow-sm relative overflow-hidden group">
                    <div className="flex justify-between items-start gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                           <span className={`text-[0.5625rem] font-black px-2 py-1 rounded-sm uppercase tracking-widest shadow-sm ${complaint.type === 'Complaint' ? 'bg-[#C8102E] text-white' : 'bg-[#003366] text-white'}`}>
                             {complaint.type || 'Complaint'}
                           </span>
                           <span className="text-[0.5625rem] font-black text-slate-300 uppercase tracking-widest select-all">GRI-ID: {complaint.id.split('-')[0]}</span>
                           {complaint.orderId && (
                             <span className="text-[0.5625rem] font-black text-[#003366] bg-slate-100 px-2 py-0.5 rounded-sm uppercase tracking-widest">TRK: {userOrders.find(o => o.id === complaint.orderId)?.trackingId}</span>
                           )}
                        </div>
                        <h3 className="text-base font-black text-[#003366] uppercase tracking-tighter leading-tight group-hover:text-[#C8102E] transition-colors">{complaint.subject}</h3>
                      </div>
                      <div className="flex flex-col items-end gap-1">
                        <div className={`px-4 py-2 border rounded-sm text-[0.625rem] font-black uppercase tracking-widest transition-all ${
                          complaint.status === 'sent' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-orange-50 text-orange-700 border-orange-200'
                        }`}>
                          {complaint.status === 'sent' ? 'RESOLVED' : 'IN PROGRESS'}
                        </div>
                      </div>
                    </div>
                         ...
                         <div className="pt-6 border-t-2 border-dashed border-slate-100">
                            <div className="flex items-center justify-between mb-4">
                               <div className="flex items-center gap-3">
                                 ...
                                 <h4 className="text-[0.625rem] font-black text-[#003366] uppercase tracking-[0.2em]">Instant Preliminary AI Resolution</h4>
                               </div>
                            </div>
                            <div className="p-8 bg-white text-[0.8125rem] leading-[2] text-slate-700 whitespace-pre-wrap rounded-sm border border-slate-100 shadow-inner font-serif relative">
                               {complaint.aiResponse}
                               <div className="mt-8 pt-4 border-t border-slate-50 flex justify-between items-center text-[0.5625rem] font-black text-slate-300">
                         ...
                         <div className="pt-6 border-t-2 border-dashed border-slate-100">
                            <div className="flex items-center justify-between mb-4">
                               <div className="flex items-center gap-3">
                                 ...
                                 <h4 className="text-[0.625rem] font-black text-[#C8102E] uppercase tracking-[0.2em]">Official Departmental Response</h4>
                               </div>
                            </div>
                            <div className="p-8 bg-white text-[0.8125rem] leading-[2] text-slate-900 whitespace-pre-wrap rounded-sm border border-[#C8102E]/20 shadow-xl font-serif relative ring-2 ring-india-post-red/5">
                               {complaint.adminResponse}
                               <div className="mt-8 pt-4 border-t border-slate-50 flex justify-between items-center text-[0.5625rem] font-black text-india-post-red">
