    ...
    <div className="max-w-md mx-auto mt-12 bg-white border border-slate-200 rounded-sm shadow-2xl relative overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="flex border-b border-slate-100 bg-slate-50">
        <button 
          ...
          className={`flex-1 py-4 text-[0.625rem] font-black uppercase tracking-[0.2em] transition-all border-b-2 ${loginMode === 'user' ? 'border-[#003366] text-[#003366] bg-white' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
        >
          {t.asUser}
        </button>
        <button 
          onClick={() => switchMode('admin')}
          className={`flex-1 py-4 text-[0.625rem] font-black uppercase tracking-[0.2em] transition-all border-b-2 ${loginMode === 'admin' ? 'border-[#C8102E] text-[#C8102E] bg-white' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
        >
          {t.asAdmin}
        </button>
      </div>
      ...
      <div className="p-10 relative">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-[0.625rem] font-bold text-slate-400 uppercase tracking-widest">
            {loginMode === 'admin' ? 'Departmental Authority Only' : 'Official Public Support Access'}
          </p>
        </div>

        <form onSubmit={handleLogin} className="space-y-6">
          <div className="space-y-1.5">
            <label className="text-[0.5625rem] font-black text-[#003366] uppercase tracking-widest">
          ...
          {error && (
            <div className="p-3 bg-red-50 border border-red-200 text-[0.625rem] font-bold text-[#C8102E] uppercase flex items-center gap-2 animate-in slide-in-from-top-2">
          ...
          <div className="pt-2">
            <button 
              type="submit"
              className={`w-full py-4 text-white text-[0.6875rem] font-black uppercase tracking-[0.3em] rounded-sm transition-all shadow-xl active:scale-95 border-b-4 border-black/10 ${
