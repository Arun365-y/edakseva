  ...
  const handleSyncGmail = async () => {
    ...
    try {
      ...
      const newMails = incoming.map(mail => ({
        ...mail, 
        customerId: mail.customerEmail, // Map legacy 'customerEmail' from mock service to 'customerId'
        source: 'gmail'
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 bg-white p-5 border-2 border-slate-100 rounded-sm shadow-sm gap-4">
           <div className="flex items-center gap-5">
              ...
              <div>
                 ...
                 <p className="text-[10px] text-slate-400 font-bold">{session.customerId}</p>
              </div>
           </div>
        ...
        <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-700">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <div className="lg:col-span-3 bg-white border-2 border-slate-100 p-6 rounded-sm shadow-sm flex flex-col justify-between">
                <div>
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Inbound Traffic</p>
                ...
                <div className="mt-8 space-y-3">
                   ...
                   <div className="w-full h-1 bg-slate-50 rounded-full overflow-hidden">
                      <div className="h-full bg-india-post-red" style={{ width: `${stats.total > 0 ? (stats.pending / stats.total) * 100 : 0}%` }}></div>
                   </div>
                   <div className="flex justify-between items-center text-[11px] font-bold">
                   ...
                   <div className="w-full h-1 bg-slate-50 rounded-full overflow-hidden">
                      <div className="h-full bg-emerald-500" style={{ width: `${stats.total > 0 ? (stats.solved / stats.total) * 100 : 0}%` }}></div>
                   </div>
                </div>
              </div>
              ...
              <div className="lg:col-span-5 bg-[#003366] text-white p-6 rounded-sm shadow-lg relative overflow-hidden">
                 ...
                 <div className="flex items-center justify-between mb-8 border-b border-white/10 pb-4">
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-[#FFCC00]">Regional Stress (Inbound Circles)</h3>
                    <div className="flex items-center gap-2">
                       <div className="w-2 h-2 rounded-full bg-india-post-red animate-pulse"></div>
                       <span className="text-[8px] font-bold text-white/40 uppercase">Real-time Feed</span>
                    </div>
                 </div>

                 <div className="grid grid-cols-2 gap-8">
                    <div>
                       <p className="text-[9px] font-black text-white/40 uppercase tracking-widest mb-3">Highest Complaining State</p>
                       {highestLocation ? (
                         <div className="flex flex-col gap-1">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                               <span className="text-[10px] font-black bg-india-post-red px-2 py-0.5 rounded-sm">{highestLocation[1]} Hits</span>
                               <span className="text-[8px] font-bold text-white/20 uppercase tracking-widest">Inundated</span>
                            </div>
                         </div>
                    ...
                    <div className="border-l border-white/10 pl-8">
                       <p className="text-[9px] font-black text-white/40 uppercase tracking-widest mb-3">Lowest Complaining State</p>
                       {lowestLocation ? (
                         <div className="flex flex-col gap-1">
                            <span className="text-xl font-black uppercase text-[#FFCC00] tracking-tight leading-none">{lowestLocation[0]}</span>
                            <div className="flex items-center gap-2 mt-1">
                               <span className="text-[10px] font-black bg-[#FFCC00] text-[#003366] px-2 py-0.5 rounded-sm">{lowestLocation[1]} Hits</span>
                               <span className="text-[8px] font-bold text-white/20 uppercase tracking-widest">Stable</span>
                            </div>
                         </div>
                       ) : <span className="text-xs text-white/20 italic">No Data Collected</span>}
                    </div>
                 ...
                 <div className="mt-8 pt-6 border-t border-white/5">
                    <p className="text-[8px] font-black text-white/30 uppercase tracking-[0.3em] mb-4">Distribution Spectrum</p>
                    <div className="space-y-3">
                       {topCircles.map(([name, count]) => (
                         <div key={name} className="flex items-center gap-4">
                            <span className="w-24 text-[9px] font-black text-white/60 uppercase truncate">{name}</span>
                            <div className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden">
                               <div className="h-full bg-india-post-red/60" style={{ width: `${(count / (highestLocation?.[1] || 1)) * 100}%` }}></div>
                            </div>
              ...
              <div className="lg:col-span-4 bg-white border-2 border-slate-100 p-6 rounded-sm shadow-sm flex flex-col justify-between overflow-hidden relative">
                  <div className="absolute top-0 right-0 p-2 bg-india-post-red text-white text-[8px] font-black uppercase -mr-2 mt-2 rotate-45 w-24 text-center">Alert Active</div>
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Priority Escalations</p>
                    <div className="flex items-center gap-4">
                       <span className="text-6xl font-black text-india-post-red">{stats.urgent}</span>
                       <div className="flex flex-col">
                          <span className="text-[10px] font-black text-india-post-red uppercase tracking-widest">Urgent (Red-Level)</span>
                          <span className="text-[8px] font-bold text-slate-400 uppercase mt-1 leading-relaxed">Requiring Post Master Sign-off</span>
                       </div>
                    </div>
                  </div>
                  <div className="mt-4 p-4 bg-red-50 border border-red-100 rounded-sm">
                     <p className="text-[9px] font-bold text-india-post-red leading-relaxed uppercase italic">
                  ...
                  {adminFilteredHistory.length === 0 ? (
                    ...
                    adminFilteredHistory.map((item) => (
                      ...
                      >
                        ...
                        <h3 className="text-[11px] font-black text-slate-800 line-clamp-1 uppercase tracking-tight">{item.customerId}</h3>
                        <p className="text-[10px] text-slate-400 font-bold line-clamp-1 mt-1">{item.subject}</p>
                        <p className="text-[8px] text-govt-blue font-black uppercase mt-2 tracking-widest opacity-60">{item.location || 'Unspecified Circle'}</p>
                      </div>
                    ))
                  )}
                </div>
                    ...
                    <div className="flex flex-col md:flex-row justify-between md:items-start gap-6 pb-6 border-b border-slate-100">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">• {currentRecord.location || 'Unspecified Circle'}</span>
                          </div>
                        </div>
                  ...
                  <div className="bg-white border-2 border-slate-100 rounded-sm shadow-2xl overflow-hidden relative">
                    ...
                    <div className="bg-slate-50 p-4 text-center border-t border-slate-100">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center justify-center gap-2">
                          ...
                          Response Processing Protocol Verified
                      </p>
                    </div>
                  </div>
                </div>
